...
Para volver a una versión anterior para un archivo utilizamos el comando `git checkout <ID commit> <nombre del archivo>`. Para voler a una versión para todos los archivos en el directorio utilizamos `git checkout <ID commit>`, si queremos guradar esos cambios entonces hacemos un `git commit`. Si queremos salir sin guradar nada entonces utilizamos el comando `git checkout master <nombre del archivo>` o `git checkout master` para el directorio completo.